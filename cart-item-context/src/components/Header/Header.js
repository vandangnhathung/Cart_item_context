...
const Header = () => {
  ...
  const likeCount =
    photos.length > 0
      ? photos.filter((photo) => photo.isLike === true).length
      : 0;
  return (
    <div className="header">
      <img
        src="https://images.unsplash.com/photo-1652013368376-11597008423d?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=774&q=80"
        alt=""
        className="header_items-img"
      />
      <div className="header_number">
        <span className="header_icon">
          ...
          <span className="header_quantity">{cartItems.length}</span>
        </span>
        <span className="header_icon">
          <svg
            width="42"
          ...
          <span className="header_quantity">{likeCount}</span>
        </span>
      </div>
    </div>
  );
